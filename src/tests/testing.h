...
#define INFO_MSG(msg, ...)                                                               \
    log_message("INFO", _test_name, __LINE__, __FILE__, (msg), __VA_ARGS__)
...
#define CHECK_EQ(x, y)                                                                   \
    CHECK_MSG(((x) == (y)), "Assertion failed: %s == %s (left=%d, right=%d)", #x, #y, x, \
              y)
#define CHECK_NE(x, y)                                                                   \
    CHECK_MSG(((x) != (y)), "Assertion failed: %s != %s (left=%d, right=%d)", #x, #y, x, \
              y)
#define CHECK_GT(x, y)                                                                   \
    CHECK_MSG(((x) > (y)), "Assertion failed: %s > %s (left=%d, right=%d)", #x, #y, x, y)
#define CHECK_GE(x, y)                                                                   \
    CHECK_MSG(((x) >= (y)), "Assertion failed: %s >= %s (left=%d, right=%d)", #x, #y, x, \
              y)
#define CHECK_LT(x, y)                                                                   \
...
#define REQUIRE_EQ(x, y)                                                                 \
    REQUIRE_MSG(((x) == (y)), "Assertion failed: %s == %s (left=%d, right=%d)", #x, #y,  \
                x, y)
#define REQUIRE_NE(x, y)                                                                 \
    REQUIRE_MSG(((x) != (y)), "Assertion failed: %s != %s (left=%d, right=%d)", #x, #y,  \
                x, y)
#define REQUIRE_GT(x, y)                                                                 \
    REQUIRE_MSG(((x) > (y)), "Assertion failed: %s > %s (left=%d, right=%d)", #x, #y, x, \
                y)
#define REQUIRE_GE(x, y)                                                                 \
    REQUIRE_MSG(((x) >= (y)), "Assertion failed: %s >= %s (left=%d, right=%d)", #x, #y,  \
                x, y)
#define REQUIRE_LT(x, y)                                                                 \
    REQUIRE_MSG(((x) < (y)), "Assertion failed: %s < %s (left=%d, right=%d)", #x, #y, x, \
                y)
#define REQUIRE_LE(x, y)                                                                 \
    REQUIRE_MSG(((x) <= (y)), "Assertion failed: %s <= %s (left=%d, right=%d)", #x, #y,  \
                x, y)
