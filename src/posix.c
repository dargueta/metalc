...
int __mclib_brk(void *new_brk) {
    void *set_brk = krnlhook_brk(new_brk, __mclib_runtime_info->udata);
...
void *__mclib_sbrk(intptr_t diff) {
    ...
    if (krnlhook_brk(new_brk, __mclib_runtime_info->udata) == NULL)
        return (void *)-1;
